...
from colette_cli.utils.validation import validate_project_name, validate_machine_name
...
class TestValidateMachineName:
    def test_valid_names(self):
        for name in VALID_NAMES:
            assert validate_machine_name(name), f"Expected '{name}' to be valid"

    def test_invalid_names(self):
        for name in INVALID_NAMES:
            assert not validate_machine_name(name), f"Expected '{name}' to be invalid"
